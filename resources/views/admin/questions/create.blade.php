        ...
        <div class="col-md-8">
            <div class="card">
                <div class="card-header">
                    <h5 class="mb-0">Создать вопрос для урока "{{ $lesson->name }}"</h5>
                </div>

                <div class="card-body">
                    <form action="{{ route('admin.lessons.questions.store', $lesson) }}" method="POST" id="questionForm">
                        @csrf
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-control @error('points') is-invalid @enderror" id="points" name="points" required>
                                ...
                                @foreach($points as $key => $value)
                                    <option value="{{ $key }}" {{ old('points') == $key ? 'selected' : '' }}>{{ $value }}</option>
                                @endforeach
                            </select>
                            @error('points')
                                <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="answers-list">
                                <div class="answer-item mb-2">
                                    <div class="input-group">
                                        ...
                                        <button type="button" class="btn btn-danger remove-answer" disabled>Удалить</button>
                                    </div>
                                </div>
                            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Добавляем второй вариант ответа при загрузке страницы
